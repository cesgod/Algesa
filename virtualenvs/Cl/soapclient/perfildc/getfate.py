...
with open('/var/www/html/dash/meters/client/production/ste.json', 'r') as f:
    rangeib = json.load(f)
nlim=len(rangeib)
for i in range(nlim):
...
sminute = str(year2)+"-"+month2+"-"+day2+"T00:00:00"
print(startdate)
print(sminute)
print(mlen)
print(day1, len(day1))
...
for i in range(lastlimit):
    allmts.append(bresponse[0]['ResultsByResultType']['ResultTypeResults'][0]['Results']['Result'][i]['Timestamp'].strftime("%d-%m-%Y %H:%M"))
    ...
    if (float(bresponse[0]['ResultsByResultType']['ResultTypeResults'][0]['Results']['Result'][i]['Value']['Value'])> float(maxdata)):
        ...
        timestamp=bresponse[0]['ResultsByResultType']['ResultTypeResults'][0]['Results']['Result'][i]['Timestamp'].strftime("%m/%d/%Y %H:%M:%S")
