...
for x in range(wlimit):
	...
	if (len(str(hour))==1):
		...
		if (mlen==1):
			...
			#print(alldvcs)
			#print(hcount)
	if (float(bresponse[0]['ResultsByResultType']['ResultTypeResults'][0]['Results']['Result'][0]['Value']['Value'])> float(maxdata)):
		maxdata=bresponse[0]['ResultsByResultType']['ResultTypeResults'][0]['Results']['Result'][0]['Value']['Value']
		timestamp=sminute
allmax=[]	
